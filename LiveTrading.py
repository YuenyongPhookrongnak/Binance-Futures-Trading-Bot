...
if __name__ == '__main__':
    ...
    # GPT Strategy specific logging
    if trading_strategy == 'GPT_Market_Analysis':
        log.info(f'GPT Configuration:\nAPI Key set: {bool(GPT_API_KEY)}\nModel: {GPT_MODEL}\n'
                 f'Min Confidence: {MIN_CONFIDENCE_SCORE}%\nMin Risk Reward: {MIN_RISK_REWARD}\n'
                 f'Max Daily Losses: {MAX_DAILY_LOSSES}\nPaper Trading: {PAPER_TRADING_MODE}')
        
        if not GPT_API_KEY:
            log.error("❌ GPT_Market_Analysis strategy requires GPT_API_KEY!")
            log.info("Please set your OpenAI API key in LiveTradingConfig.py")
            if FALLBACK_TO_SIMPLE_STRATEGY:
                log.info(f"Falling back to simple strategy: {SIMPLE_STRATEGY_CONFIG['strategy']}")
                trading_strategy = SIMPLE_STRATEGY_CONFIG['strategy']
            else:
                sys.exit(1)
    ...
    # แก้ไข type annotation - ใช้ List แทน [BotClass.Bot]
    Bots: List[BotClass.Bot] = []
    signal_queue = None
    print_trades_q = None
    ...
    # GPT Strategy specific initialization
    if trading_strategy == 'GPT_Market_Analysis':
        log.info("🤖 GPT Market Analysis Strategy Initialized!")
        log.info("📊 Waiting for sufficient market data before analysis...")
        log.info("⚡ Will start generating signals once buffer is complete")
        
        # Optional: Test GPT API connection
        try:
            from TradingStrats import call_gpt_api
            test_data = {
                "symbol": "TEST",
                "test": "API connection test",
                "request": "Please respond with: 'API connection successful'"
            }
            test_result = call_gpt_api(test_data)
            if test_result:
                log.info("✅ GPT API connection test successful")
            else:
                log.warning("⚠️ GPT API connection test failed - check your API key")
        except Exception as e:
            log.warning(f"⚠️ GPT API test failed: {e}")
