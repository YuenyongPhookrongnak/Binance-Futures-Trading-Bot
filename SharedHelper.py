import numpy as np
from Logger import *
import BotClass  # แก้ไขการ import
import sys
import os
...
def get_required_buffer(trading_strategy):
    ...
    # GPT Strategy specific buffer calculation
    if trading_strategy == 'GPT_Market_Analysis':
        log.info('get_required_buffer() - GPT Strategy detected, using recommended buffer of 300 candles')
        return 300  # GPT strategy needs sufficient data for technical analysis
    
    # Legacy buffer calculation for other strategies
    try:
        ...
        # Fallback buffer for unknown strategies
        log.warning(f"get_required_buffer() - Could not calculate optimal buffer for {trading_strategy}, using default 250 candles")
        return 250
        
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        log.error(f"get_required_buffer() - Critical error: {exc_obj, fname, exc_tb.tb_lineno}, Error: {e}")
        return 200  # Emergency fallback